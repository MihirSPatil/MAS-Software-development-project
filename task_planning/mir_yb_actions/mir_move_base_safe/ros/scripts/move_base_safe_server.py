...
class AlignWithWorkspace(smach.State): # inherit from the State base class
    def __init__(self):
        ...
        if self.align:
            self.client = actionlib.SimpleActionClient('/align_with_workspace_server', AlignWithWorkspaceAction)
    ...
    def execute(self, userdata):
        ...
        goal.goal.destination_location = userdata.move_base_safe_goal.destination_location
        timeout = 10.0
...
class CheckDontBeSafe(smach.State): # inherit from the State base class
    def __init__(self):
        smach.State.__init__(self,  outcomes=['safe','unsafe'], 
                                    input_keys=['move_base_safe_goal'])

    def execute(self, userdata):
        if userdata.move_base_safe_goal.dont_be_safe:
            return 'unsafe'
        else:
            return 'safe'
...
#                transitions={'success':'SETUP_MOVE_ARM'})
        ...
        smach.StateMachine.add('START_BARRIER_TAPE_DETECTION', gbs.send_event([('/mcr_perception/barrier_tape_detection/event_in', 'e_trigger')]),
                transitions={'success':'SETUP_MOVE_ARM'})
        ...
        smach.StateMachine.add('MOVE_ARM', gms.move_arm(),
                transitions={'succeeded': 'SETUP_MOVE_BASE',
                             'failed': 'MOVE_ARM'})
        '''
        smach.StateMachine.add('MOVE_ARM', gms.move_arm_and_gripper('open_wide'),
                transitions={'succeeded': 'SETUP_MOVE_BASE',
                             'failed': 'MOVE_ARM'})
        ...
        smach.StateMachine.add('STOP_BARRIER_TAPE_DETECTION_WITH_FAILURE', gbs.send_event([('/mcr_perception/barrier_tape_detection/event_in', 'e_stop')]),
                transitions={'success':'SET_ACTION_LIB_FAILURE'})

        smach.StateMachine.add('STOP_BARRIER_TAPE_DETECTION_WITH_SUCCESS', gbs.send_event([('/mcr_perception/barrier_tape_detection/event_in', 'e_stop')]),
