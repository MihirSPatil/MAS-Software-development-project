...
if __name__ == '__main__':
    rospy.init_node('pick_from_rtt_client_tester')
    client = actionlib.SimpleActionClient('pick_object_rtt_server', PickObjectAction)
    client.wait_for_server()
    goal = PickObjectGoal()
    if len(sys.argv) == 1: # 1 arguments was received : ok proceed
        try:
            goal.object = 'ANY'
            timeout = 60.0
