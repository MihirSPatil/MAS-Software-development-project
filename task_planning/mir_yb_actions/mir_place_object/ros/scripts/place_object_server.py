...
class GetPoseToPlaceOject(smach.State): # inherit from the State base class
    ...
    def execute(self, userdata):
        ...
        location = userdata.place_object_goal.location.upper()
        ...
        while not(rospy.is_shutdown()):
            if (rospy.Time.now() - start_time > self.timeout):
                break
            if self.message_received and self.status_received:
                break
            rate.sleep()
...
class send_event(smach.State):
    ...
    def execute(self, userdata):
        ...
        # filling message
        msg.data = self.event.upper()
        # publish
        self.publisher.publish(msg)
        rospy.loginfo('publishing on ' + self.topic_name + ' ' + msg.data)
