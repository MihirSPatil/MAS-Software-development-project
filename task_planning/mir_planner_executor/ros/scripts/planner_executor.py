...
current_action_pub = rospy.Publisher('~current_action', String, queue_size=1)
...
def insert_object(o1, o2, robot_platform, duration=150.0):
    """
    o1, o2 are strings, they describe the objects which needs to be inserted
    inserts object1 into object2
    """
    print('insert_object ' + o1 + ' in ' + o2 + ' unstage from ' + robot_platform)
    # mockup begins----
    if mockup:
...
def update_kb_pick_object(obj, location, gripper, success=True):
    if success:
        ...
        try:
            ...
            timeout = 15.0
            rospy.loginfo('Sending action lib goal to after pick action failure move_base_safe_server, source : ' + goal.source_location + ' , destination : ' + goal.destination_location)
            client.send_goal(goal)
            client.wait_for_result(rospy.Duration.from_sec(int(timeout)))
            print client.get_result()
            #wiggle_base_pub.publish('e_trigger')
        except:
            rospy.logwarn("Exception: while sending move base goal after pick action failure")

def update_kb_insert_object(peg, hole, gripper, location, robot_platform, success=True):
    if success:
        ...
        params = [['peg', peg.upper()],['hole', hole.upper()]]
        rosplan_update_knowledge(1, 'n/a', 'n/a', 'in', params, update_type='ADD_KNOWLEDGE')
        rosplan_update_knowledge(1, 'n/a', 'n/a', 'in', params, update_type='REMOVE_GOAL')
        params = [['peg', peg.upper()],['l', location.upper()]]
        ...
        params = [['hole', hole.upper()]]
        rosplan_update_knowledge(1, 'n/a', 'n/a', 'heavy', params, update_type='ADD_KNOWLEDGE')
        rosplan_update_knowledge(1, 'n/a', 'n/a', 'heavy', params, update_type='REMOVE_GOAL')
        params = [['o', peg],['rp', robot_platform]]
...
def update_kb_perceive(location, success=True):
    if success:
        ...
        try:
            ...
            goal.destination_location = location
            timeout = 15.0
            rospy.loginfo('Sending action lib goal after percieve action failure to move_base_safe_server, source : ' + goal.source_location + ' , destination : ' + goal.destination_location)
            client.send_goal(goal)
            client.wait_for_result(rospy.Duration.from_sec(int(timeout)))
            print client.get_result()
            #wiggle_base_pub.publish('e_trigger')
        except:
            rospy.logwarn("Exception: while sending move base goal after percieve action failure")

def update_kb_place_object(obj, location, gripper, success=True):
    if success:
        params = [['o', obj.upper()],['l', location.upper()]]
        rosplan_update_knowledge(1, 'n/a', 'n/a', 'on', params, update_type='ADD_KNOWLEDGE')
        rosplan_update_knowledge(1, 'n/a', 'n/a', 'on', params, update_type='REMOVE_GOAL')
        params = [['g', gripper],['o', obj]]
        rosplan_update_knowledge(1, 'n/a', 'n/a', 'holding', params, update_type='REMOVE_KNOWLEDGE')
...
def planCallBack(complete_plan):
    ...
    time_log = open('/home/robocup/.ros/timing.log', 'w+')
    for action in complete_plan.plan:
        print(action_failed_count)
        failed_count = action_failed_count.get(action.name, {})
        action_failed_count[action.name] = failed_count
        ...
        if action.name == 'move_base_safe' or action.name == 'move_base_safe_to_pick' or action.name == 'move_base_safe_to_deliver':
            ...
            msg = str(action.name) + " to " + str(action.parameters[1].value)
            current_action_pub.publish(msg)
            ...
            obj = action.parameters[0].value
            loc = action.parameters[1].value
            msg = str(action.name) + " " + str(loc)
            current_action_pub.publish(msg)
            if perceive_location(loc, obj, duration=150.0):
                rospy.loginfo('action succeded !')
                update_kb_perceive(loc)
            else:
                update_kb_perceive(loc, False)
                rospy.logerr('perceive action failed')
                return
        elif action.name=='pick':
            msg = str(action.name) + " " + str(action.parameters[0].value)
            current_action_pub.publish(msg)
            ...
            object_name = action.parameters[0].value
            count = failed_count.get(object_name, 0)
            if pick_object_wbc(action.parameters[0].value, duration=150.0):
                rospy.loginfo('pick wbc action succeded !')
                update_kb_pick_object(action.parameters[0].value, action.parameters[1].value, action.parameters[3].value)
                failed_count[object_name] = 0
            else:
                if count < 2: #1 tries
                    rospy.logwarn('pick wbc action failed!')
                    failed_count[object_name] = count + 1
                    update_kb_pick_object(action.parameters[0].value, action.parameters[1].value, action.parameters[3].value, False)
                else:
                    rospy.logwarn('pick wbc action failed THREE  times, removing goal!')
                    goals_client = rospy.ServiceProxy('/kcl_rosplan/get_current_goals', rosplan_service.GetAttributeService)
                    resp = goals_client('')
                    for goal in resp.attributes:
                        for item in goal.values:
                            if (item.key.lower() == 'o' or item.key.lower() == 'peg') and (item.value.upper() == object_name.upper()):
                                update_client = rospy.ServiceProxy('/kcl_rosplan/update_knowledge_base', rosplan_service.KnowledgeUpdateService)
                                resp = update_client(rosplan_service.KnowledgeUpdateServiceRequest.REMOVE_GOAL, goal)
                    print(resp)
                    #update_kb_pick_object(action.parameters[0].value, action.parameters[1].value, action.parameters[3].value, False)
                return
        elif action.name=='stage':
            msg = str(action.name) + " " + str(action.parameters[1].value)
            current_action_pub.publish(msg)
            ...
            msg = str(action.name) + " " + str(action.parameters[0].value) + " into " + str(action.parameters[1].value) + " from " + str(action.parameters[5].value)
            current_action_pub.publish(msg)
            rospy.loginfo('insert action')
            # o1, o2, robot_platform
            if insert_object(action.parameters[0].value, action.parameters[1].value, action.parameters[5].value, duration=150.0):
                rospy.loginfo('action succeded !')
                update_kb_insert_object(action.parameters[0].value, action.parameters[1].value, action.parameters[2].value, action.parameters[4].value, action.parameters[5].value)
            else:
                rospy.logwarn('insert action failed!')
                update_kb_insert_object(action.parameters[0].value, action.parameters[1].value, action.parameters[2].value, action.parameters[4].value, action.parameters[5].value, False)
                return
        elif action.name=='place':
            rospy.loginfo('place_object')
            msg = str(action.name) + " " + str(action.parameters[0].value) + " at " + str(action.parameters[1].value)
            current_action_pub.publish(msg)
        ...
        time_log.write(action.name + ': ' + str(seconds) + ' s\n')
        time_log.flush()
